...
dir.create("~/buckets/b1/exp/lineademuertev5/", showWarnings = FALSE)
setwd( "~/buckets/b1/exp/lineademuertev5/" )
...
dataset[, fold_train :=  foto_mes <= 202107 &
          !(foto_mes %in% c(201903, 201904, 202002, 202003, 202004, 202103, 202104)) &
          (clase_ternaria %in% c("BAJA+1", "BAJA+2") |
             azar < 0.2 ) ]
...
# --- Code Cell ---
# Genero la tabla de entrega con predicciones binarias
...
# Archivo de predicciones binarias
tb_entrega[, Predicted := 0L]
tb_entrega[1:11000, Predicted := 1L]

fwrite(tb_entrega[, .(numero_de_cliente, Predicted)], 
       file = "lineademuerte_11000v5.csv")

# --- Code Cell ---
# Genero el archivo de entrega con probabilidades
tb_entrega_prob <- dfuture[, list(numero_de_cliente)]
tb_entrega_prob[, prob := prediccion]

# Ordeno por probabilidad descendente (opcional)
setorder(tb_entrega_prob, -prob)

# Guardo el archivo con probabilidades
fwrite(tb_entrega_prob, file = "lineademuerte_probabilidades_v5.csv")
