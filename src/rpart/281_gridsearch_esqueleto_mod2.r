...
dir.create("~/maest_2024/eyf/exp/HT2810/", showWarnings = FALSE)
setwd( "~/maest_2024/eyf/exp/HT2810/" )
...
for (vcp in c(-1, -0.5, 0.5, 1)) {       # Loop para cp
  for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {  # Loop para maxdepth
    for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {  # Loop para minsplit
      ...
      # Calcular valores de minbucket entre un tercio y la mitad de minsplit
      posibles_min_buckets <- seq(ceiling(vmin_split / 3), floor(vmin_split / 2))
      
      for (vmin_bucket in posibles_min_buckets) {  
